...
impl Square {
    ...
    pub fn new(width: f32, height: f32, x: f32, y: f32, color: [f32; 4]) -> Square {
        ...
        let bottom_right = Coordinate {x + width, y - height};
...
pub fn render(config: &super::Config, view: &super::View, board: Vec<Vec<super::game_of_life::CellState>>, alive_color: [f32; 4], dead_color: [f32; 4]) {
    ...
    for board_y in 0..view.cells_on_height {
        for board_x in 0..view.cells_on_width {
            if board[board_y + view.y][board_x + view.x] == super::CellState::Alive {
                let square = Square::simple(view, board_x, board_y, alive_color);
                square.draw();
            } else if config.view_border && (board_y + view.y == 0 || board_y + view.y + 1 == view.board_height || board_x + view.x == 0 || board_x + view.x + 1 == view.board_width) {
