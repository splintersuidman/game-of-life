...
impl Renderer {
    ...
    pub fn render(&self, config: &Config, view: &View, game_of_life: &GameOfLife) {
        ...
        assert!(view.cells_on_width - 1 + view.round_x() < view.board_width);
        assert!(view.cells_on_height - 1 + view.round_y() < view.board_height);

        for board_y in 0..view.cells_on_height {
            for board_x in 0..view.cells_on_width {
                if game_of_life.board[board_y + view.round_y()][board_x + view.round_x()]
                    == CellState::Alive
                    || (config.view_border
                        && (board_y + view.round_y() == 0
                            || board_y + view.round_y() + 1 == view.board_height
                            || board_x + view.round_x() == 0
                            || board_x + view.round_x() + 1 == view.board_width))
                {
